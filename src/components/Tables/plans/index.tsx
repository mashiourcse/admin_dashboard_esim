...
import { Table, Button, Space, ConfigProvider, Modal, Card, Row, Form, Input, message, Switch } from 'antd';
...
interface PlanData {
  ...
  status: boolean; // true for active, false for disabled
}
      ...
      render: (_: any, record: PlanData) => (
        <Space size="middle">
          ...
          <Switch
            ...
            unCheckedChildren=""
          />
          <Button type="link" icon={<DeleteOutlined className="text-red-500" />} onClick={() => handleDeleteClick(record)} />
        </Space>
      ),
    },
  ];
          ...
          <Card title="Plan Details" bordered={true} style={{ width: '100%' }}>
            <Form layout="vertical">
              <Form.Item label="Plan Name">
                <Input name="planName" value={formData.planName} readOnly />
              </Form.Item>
              <Form.Item label="Selling Price">
                <Input name="sellingPrice" value={formData.sellingPrice} readOnly />
              </Form.Item>
              ...
              <Form.Item label="Data/Validity">
                <Input name="dataOrValidity" value={formData.dataOrValidity} readOnly />
              </Form.Item>
            </Form>
          </Card>
        )}
